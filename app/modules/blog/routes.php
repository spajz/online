...
if ($json['enabled']) {
    ...
    extract(Config::get('blog::module', array()));
    ...
    // Front routes
    Route::get("inspiration", array("as" => "{$moduleLower}.index", "uses" => $moduleUpper . '\Controllers\\' . "{$moduleUpper}Controller@index"));

    Route::get("inspiration/{slug}", array("as" => "{$moduleLower}.show", "uses" => $moduleUpper . '\Controllers\\' . "{$moduleUpper}Controller@show"));
