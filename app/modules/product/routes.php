...
if ($json['enabled']) {
    ...
    extract(Config::get('product::module', array()));
    ...
    Route::group(array("prefix" => "admin", "before" => "admin"), function () use ($moduleUpper, $moduleLower) {
        ...
        Route::get("{$moduleLower}/attribute/{id}/destroy", array("as" => "admin.{$moduleLower}.attribute.destroy", "uses" => $moduleUpper . '\Admin\Controllers\\' . "{$moduleUpper}Controller@destroyAttribute"));
