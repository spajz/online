...
if ($json['enabled']) {
    ...
    extract(Config::get('quiz::module', array()));
    ...
    // Front routes
    Route::get("quiz", array("as" => "{$moduleLower}.index", "uses" => $moduleUpper . '\Controllers\\' . "{$moduleUpper}Controller@index"));

    Route::get("quiz/{slug}", array("as" => "{$moduleLower}.show", "uses" => $moduleUpper . '\Controllers\\' . "{$moduleUpper}Controller@show"));
